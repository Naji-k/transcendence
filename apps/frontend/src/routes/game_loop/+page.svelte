...
<!-- Main Container with matching dark gradient -->
<div class="min-h-screen font-['Press_Start_2P'] bg-gradient-to-br from-[#0f2027] via-[#203a43] to-[#2c5364] py-8">
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    
    <!-- Header Section with retro gaming style -->
    <div class="text-center mb-8">
      <h1 class="text-5xl font-bold text-cyan-400 drop-shadow-lg mb-4">GAME LOBBY</h1>
      <p class="text-sm text-cyan-300 tracking-wide">Create or join a multiplayer match</p>
    </div>

    <!-- Error Alert with matching theme -->
    {#if error}
    ...
    <!-- Actions Section with retro gaming cards -->
    ...
    <!-- Games Table Section with retro gaming style -->
    <div class="bg-gradient-to-br from-purple-700 to-indigo-900 rounded-3xl shadow-2xl overflow-hidden">
      <div class="px-6 py-4 border-b-2 border-cyan-400/30">
        <h2 class="text-lg font-bold text-cyan-400 text-center">AVAILABLE GAMES</h2>
      </div>
      ...
      {:else if games.length === 0}
        ...
        <div class="p-12 text-center">
          ...
          <h3 class="mt-4 text-sm font-bold text-cyan-300">NO GAMES AVAILABLE</h3>
          <p class="mt-2 text-xs text-cyan-400/70">Create a new game to start!</p>
        </div>
      {:else}
        <!-- Games Table -->
        <div class="overflow-x-auto">
          <table class="min-w-full">
            ...
            <tr>
              ...
              <th class="px-6 py-4 text-right text-xs font-bold text-cyan-400 uppercase tracking-wider">
                ACTIONS
              </th>
            </tr>
            </thead>
            ...
            {#each games as game}
              <tr class="hover:bg-black/20 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-xs font-bold text-white">GAME #{game.id}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center gap-2">
                    ...
                    {#if game.playerCount === game.maxPlayers}
                      <span class="inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-bold bg-green-500 text-black">
                        FULL
                      </span>
                    {:else}
                      <span class="inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-bold bg-yellow-500 text-black">
                        OPEN
                      </span>
                    {/if}
                  </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-bold uppercase
                    {game.status === 'waiting' ? 'bg-yellow-500 text-black' :
                     game.status === 'ready' ? 'bg-green-500 text-black' :
                     game.status === 'playing' ? 'bg-cyan-500 text-black' :
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right">
                  <div class="flex justify-end gap-2">
                    ...
                    {#if game.status === 'playing'}
                      <span class="inline-flex items-center px-4 py-2 text-[10px] font-bold text-cyan-400/50">
                        IN PROGRESS
                      </span>
                    {/if}
                  </div>
