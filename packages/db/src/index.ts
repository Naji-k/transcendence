...
import { matchHistoryTable, singleMatchParticipantsTable, usersTable } from './db_schema/schema';
...
async function main() {
	...
	/* Test user entries */
	try {
		await db.insert(usersTable).values({ alias: "first", password: `pass${Date.now()}`, name: "name_1", email: "example1@example.com"});
		await db.insert(usersTable).values({ alias: "second", password: `pass${Date.now()}`, name: "name_2", email: "example2@example.com"});
		await db.insert(usersTable).values({ alias: "third", password: `pass${Date.now()}`, name: "name_3", email: "example3@example.com"});
		await db.insert(usersTable).values({ alias: "fourth", password: `pass${Date.now()}`, name: "name_4", email: "example4@example.com"});
		// error user
		// await db.insert(usersTable).values({ alias: "fourth", password: `pass${Date.now()}`, name: "name_4", email: "example4@example.com"});
	} catch (error) {
